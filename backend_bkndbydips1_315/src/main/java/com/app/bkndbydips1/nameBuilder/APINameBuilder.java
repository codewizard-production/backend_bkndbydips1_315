package com.app.bkndbydips1.nameBuilder;
...
public class APINameBuilder implements JPAEdmNameBuilder {
	...
	public String buildEntitySetName(final String entityTypeName) {
    	...
    	eSetName = "Training".equals(entityTypeName) ? "Trainings" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "PerformanceAppraisal".equals(entityTypeName) ? "PerformanceAppraisals" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Organization".equals(entityTypeName) ? "Organizations" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "EmployeePersonalInfo".equals(entityTypeName) ? "EmployeePersonalInfoes" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "EmlpoyeeProfile".equals(entityTypeName) ? "EmlpoyeeProfiles" : defaultNameBuilder.buildEntitySetName(entityTypeName);
        return eSetName;
    }
